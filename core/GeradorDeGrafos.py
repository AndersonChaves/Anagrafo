...
import math
from Starlike import Starlike

class GeradorDeGrafos():

    def gerar_double_broom_como_matriz_de_adjacencias(self, k, l, diametro):
        m = k + l + (diametro - 1)
        n = m
        matriz = np.zeros((m, m))
    ...
    def gerar_double_broom_como_nx(self, k, l, diametro):
        return nx.from_numpy_matrix(self.gerar_double_broom_como_matriz_de_adjacencias(k, l, diametro))

    def gerar_double_broom(self, k, l, diametro, nome=""):
        grafo_nx = self.gerar_double_broom_como_nx(k, l, diametro)
        return DoubleBroom(nx.Graph(grafo_nx), diametro, grafo_nx.order(), k, l, nome)
    ...
    def gerar_broom(self, diametro, n, nome=""):
        ...
        return self.gerar_double_broom(k, l, diametro, nome)

    def gerar_double_brooms_por_ordem_e_diametro(self, ordem, diametro):
        lista_de_grafos = []
        ...
        for k in range(1, int(k_maximo + 1)):
            ...
            arvore_t = self.gerar_double_broom(k, l, diametro, nome_do_grafo)
            lista_de_grafos.append(arvore_t)
        return lista_de_grafos


    def gerar_listas_de_double_brooms_por_diametro_variando_ordem(self, *args):
        if len(args) == 3:
        ...
        for ordem in range(ordem_minima, ordem_maxima + 1):
            ...
            lista_de_grafos.append(self.gerar_double_brooms_por_ordem_e_diametro(ordem, diametro))
        return lista_de_grafos

    def gerar_listas_de_double_brooms_por_diametro_variando_ordem_com_k_fixado(self, ordem_maxima, diametro, k, ordem_minima = 0):
        if ordem_minima == 0:
            ordem_minima = diametro + 1
        print "Gerando grafos de diametro " + str(diametro)
        lista_de_grafos = []
        for ordem in range(ordem_minima, ordem_maxima + 1):
            ...
            nome_do_grafo = "T(" + str(ordem) + ", " + str(diametro) + ", " + str(k) + ")"
            lista_de_grafos.append(self.gerar_double_broom(k, l, diametro, nome_do_grafo))
        return lista_de_grafos
    ...
    def gerar_starlike(self, tamanhos_dos_ramos):
        ...
        for i in range(len(tamanhos_dos_ramos)):
            ...
            if i < len(tamanhos_dos_ramos)-1:
                nome += ', '
        nome += ')'
        return Starlike(nx.Graph(grafo_nx), nome)
