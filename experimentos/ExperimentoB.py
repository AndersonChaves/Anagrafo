...
class ExperimentoB(ExperimentoHeuristicaMacap):
    ...
    def gerar_grafos(self):
        ...
        lista_de_grafos.append(gerador_de_grafos.gerar_arvore_t(1, 66, 67, "T(1, 66, 66)"))
    ...
    def _gerar_dicionario_de_resultados_para_grafo(self, grafo):
        ...
        dicionario_de_resultados["novo_grafo_fb2"] = grafo.copia().adicionar_aresta(dicionario_de_resultados["aresta_fb2"])
    ...
    def _adicionar_linha_de_resultados(self, grafo, dicionario_de_resultados):
        conectividade_hp = grafo.copia().adicionar_aresta(dicionario_de_resultados["aresta_hp"]).obter_conectividade_algebrica()
        conectividade_he = grafo.copia().adicionar_aresta(dicionario_de_resultados["aresta_he"]).obter_conectividade_algebrica()
        conectividade_fb = grafo.copia().adicionar_aresta(dicionario_de_resultados["aresta_fb"]).obter_conectividade_algebrica()
        conectividade_fb2 = grafo.copia().adicionar_aresta(dicionario_de_resultados["aresta_fb2"]).obter_conectividade_algebrica()
...
'''grafo = GeradorDeGrafos().gerar_arvore_t(1, 38, 38, "T(1, 38, 38)")
print grafo.obter_conectividade_algebrica()
print grafo.copia().adicionar_aresta((10, 70)).obter_conectividade_algebrica()
print grafo.copia().adicionar_aresta((0, 37)).obter_conectividade_algebrica()


DesenhistaDeGrafos().plotar_grafo_na_tela(grafo.copia().adicionar_aresta((0, 37)))
DesenhistaDeGrafos().plotar_grafo_na_tela(grafo)

grafo = GeradorDeGrafos().gerar_arvore_t(1, 56, 56, "T(1, 56, 56)")
print grafo.obter_conectividade_algebrica()
print grafo.copia().adicionar_aresta((15, 109)).obter_conectividade_algebrica()
print grafo.copia().adicionar_aresta((15, 55)).obter_conectividade_algebrica()
'''
#grafo = GeradorDeGrafos().gerar_arvore_t(1, 56, 56, "T(1, 56, 56)")
#print grafo.obter_conectividade_algebrica()
#print grafo.copia().adicionar_aresta((15, 110)).obter_conectividade_algebrica()

grafo = GeradorDeGrafos().gerar_arvore_t(1, 56, 56, "T(1, 56, 56)")


#print grafo.adicionar_aresta((10, 37)).obter_conectividade_algebrica()
...
for aresta in lista:
    print aresta, grafo.copia().adicionar_aresta(aresta).obter_conectividade_algebrica()
'''
...
print grafo.obter_conectividade_algebrica()
print grafo.copia().adicionar_aresta((15, 55)).obter_conectividade_algebrica()
print grafo.copia().adicionar_aresta((15, 56)).obter_conectividade_algebrica()
...
#print ExperimentoB().gerar_grafos().adicionar_aresta((10, 70)).obter_conectividade_algebrica()
#print ExperimentoB().gerar_grafos().adicionar_aresta((10, 37)).obter_conectividade_algebrica()
